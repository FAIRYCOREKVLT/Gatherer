import random, string, os, requests, hashlib, threading, time
...
def keygen_unique(number_of_symbols):
	random_api_key = ''.join(random.choices(string.ascii_lowercase + string.digits, k=number_of_symbols))
	unique_key = hashlib.sha256(random_api_key.encode()).hexdigest()[:number_of_symbols] # hashlib for unique symbol combinations
	...
	try:
		...
		if donecheck.status_code == 400: # error 401 says about wrong API key. set 401 to test key saving
			save_to_file([unique_key + " "], "hunter_key.txt") # error 400 says about wrong domain but existing API key, so it can be used to define the right key
			...
			while donecheck.status_code == 403: # trying the same key if 403 error
				time.sleep(0.6) # lowering request frequency
				url = "https://api.hunter.io/v2/email-finder?domain=gmail.com&first_name=test&last_name=test&api_key=" + unique_key
